...
export function initSchema() {
  ...
  try { db.exec(`ALTER TABLE orders ADD COLUMN payment_method TEXT DEFAULT 'local'`); } catch (_) {}
  try { db.exec(`ALTER TABLE orders ADD COLUMN payment_status TEXT DEFAULT 'pending'`); } catch (_) {}
  try { db.exec(`ALTER TABLE orders ADD COLUMN stripe_session_id TEXT`); } catch (_) {}
  try { db.exec(`ALTER TABLE orders ADD COLUMN total_amount REAL`); } catch (_) {}
...
export function createOrder({ 
  ...
  const stmt = db.prepare(`
    INSERT INTO orders (product_service_id, customer_id, address, phoneno, status, scheduled_date, payment_method, payment_status, stripe_session_id, total_amount)
    VALUES (@product_service_id, @customer_id, @address, @phoneno, @status, @scheduled_date, @payment_method, @payment_status, @stripe_session_id, @total_amount)
  `);
  return stmt.run({ 
    product_service_id, 
    customer_id, 
...
export function updateOrder(id, { address, phoneno, scheduled_date, payment_method, payment_status, stripe_session_id, total_amount }) {
  const updates = [];
  const params = { id };
  ...
  if (stripe_session_id !== undefined) {
    updates.push("stripe_session_id = @stripe_session_id");
    params.stripe_session_id = stripe_session_id;
  }
...
export function updateOrderPayment(id, { payment_status, stripe_session_id }) {
  const stmt = db.prepare(`
    UPDATE orders 
    ...
    WHERE id = @id
  `);
  return stmt.run({ id, payment_status, stripe_session_id: stripe_session_id || null });
}
...
export function getOrdersByStripeSessionId(sessionId) {
  if (!sessionId) return [];
  return db.prepare(`
    SELECT 
      o.id AS order_id,
